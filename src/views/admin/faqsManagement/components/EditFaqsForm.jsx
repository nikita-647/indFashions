// src/views/admin/faqsManagement/components/EditFaqsForm.jsx
import React, { useState } from "react";
import { IoClose } from "react-icons/io5";
import { FaQuestionCircle, FaList, FaSort, FaCheck } from "react-icons/fa";
import toast from "react-hot-toast";

const EditFaqsForm = ({ faq, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // const response = await axios.put(`YOUR_API_ENDPOINT/faqs/${faq._id}`, formData);

      // Simulate success
      setTimeout(() => {
        toast.dismiss(loadingToast);
        toast.success("FAQ updated successfully");
        onUpdate({ ...faq, ...formData });
      }, 1000);
    } catch (error) {
      toast.dismiss(loadingToast);
  ...
  return (
    <div className="bg-black/60 fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-4 backdrop-blur-sm transition-all duration-300">
      <div className="my-4 w-full max-w-2xl rounded-2xl border border-blue-100 bg-white shadow-2xl sm:my-6">
        {/* Header */}
        <div className="border-b border-blue-100 bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-3 sm:px-6 sm:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/90">Update FAQ information</p>
              </div>
            </div>
            <button
              onClick={onClose}
